...
def generate_3trace_s6p(filename, f_min, f_max, f_points, layer, **kwargs):
    """
    Generates a 3-trace (.s6p) Touchstone file using the EMerge solver.
    Uses the global pitch for trace spacing and layer length expansion.
    """
    print(f"[EMerge] Meshing and solving 3-trace baseline: {filename}...")
    ...
    # 3. 3D Model Setup
    model = em.Simulation('ThreeTrace')
    ...
    # 4. Traces (Center trace at 0, outer traces at +/- pitch_um)
    pcb.new(0, pitch_um, w0, (1,0), trace_bottom).store('p1').straight(trace_length).store('p2')
    pcb.new(0, 0, w0, (1,0), trace_bottom).store('p3').straight(trace_length).store('p4')
    pcb.new(0, -pitch_um, w0, (1,0), trace_bottom).store('p5').straight(trace_length).store('p6')
    ...
    lp5 = pcb.lumped_port(pcb.load('p5'))
    lp6 = pcb.lumped_port(pcb.load('p6'))
    ...
    model.mw.bc.LumpedPort(lp5, 5)
    model.mw.bc.LumpedPort(lp6, 6)
    
    data = model.mw.run_sweep(True, 2)  
